...
type Reader struct {
	...
	// objCache memoizes resolved indirect objects by pointer.
	//
	// Upstream ships without one — its own doc comment reads "the library
	// makes no attempt at efficiency; a value cache maintained in the Reader
	// would probably help significantly" — and the cost is not marginal.
	// Content() calls Font.Width once per glyph, and each call re-reads and
	// re-lexes the font's /Widths array straight out of the file bytes, so
	// laying out a page is O(glyphs x len(Widths)) trips through the lexer.
	// On markitdown's test.pdf that is 1.57M allocations and 75 MB for a 93 KB
	// document; with the cache it is 16K allocations and 1.9 MB, 13x faster.
	//
	// Correctness is unaffected: a Reader is read-only, an objptr identifies
	// exactly one object in one file, and resolve() is deterministic — so a
	// hit returns what a miss would have parsed. The cache is bounded by the
	// number of distinct objects the document actually references, which the
	// xref table already bounds.
	objCache map[objptr]interface{}
}
...
func (r *Reader) resolve(parent objptr, x interface{}) Value {
	if ptr, ok := x.(objptr); ok {
		...
		if v, ok := r.objCache[ptr]; ok {
			return Value{r, ptr, v}
		}
		var obj object
		if xref.inStream {
		...
		if r.objCache == nil {
			r.objCache = make(map[objptr]interface{})
		}
		r.objCache[ptr] = x
		parent = ptr
	}
